...
|---|---|---|
| Skill documents | Planning and reviewing workflows around named regions, steps, fields, loads, meshes, interactions, outputs, and ODBs | Equivalent runtime behavior across every Abaqus/CAE release |
...
| Repository tests | Python 3.10+ structural, link, privacy, and documentation checks without Abaqus | Solver convergence, physical correctness, or mesh independence |
...
3. Keep read-only checks and dry-run descriptions separate from authorized
   model changes.
4. Run the repository tests and document any release-specific exception in a
